...
const writeItems = (module:string,items:CmsItem[]) => cmsService.save(module,items)
...
export function AdminHome() {
  const stats = [['Projects','projects',FolderKanban],['Clients','clients',UsersRound],['Blog Posts','blogs',FileText],['Testimonials','testimonials',MessageSquareQuote],['Open Positions','hiring',BriefcaseBusiness],['Team Members','team',UserRound]] as const
  ...
  return <div className="cms-page"><PageHeader eyebrow="Dashboard" title="Welcome back, Admin" copy="Manage your Unseen Studios website content."/><section className="cms-stat-grid">{stats.map(([label,key,Icon])=>{const items=readItems(key);const count=key==='hiring'?items.filter((item)=>item.status==='Open').length:items.length;return <Link to={`/admin/${key}`} className="cms-stat-card" key={key}><div><span>{label}</span><strong>{String(count).padStart(2,'0')}</strong></div><Icon/></Link>})}</section><section className="cms-panel"><div className="cms-panel-head"><div><p className="cms-kicker">Quick actions</p><h2>Create new content</h2></div></div><div className="cms-quick-grid">{actions.map(([label,key])=><Link to={`/admin/${key}/new`} key={key}><Plus/>{label}<span>↗</span></Link>)}</div></section></div>
}
...
export function ContentManager({module}:{module:CmsModule}) {
  ...
  const [items,setItems]=useState<CmsItem[]>(()=>readItems(module)),[search,setSearch]=useState(''),[filter,setFilter]=useState('All'),[deleteItem,setDeleteItem]=useState<CmsItem|null>(null),[previewItem,setPreviewItem]=useState<CmsItem|null>(null),[message,setMessage]=useState('')
  ...
  const persist=(next:CmsItem[])=>{setItems(next);writeItems(module,next);void cmsService.saveRemote(module,next).catch(()=>setMessage('Saved on this device. Start the API server to publish this change everywhere.'))}
  if(isEditor)return <ContentEditor module={module} existing={editing} onSave={(item)=>{persist(editing?items.map((entry)=>entry.id===editing.id?item:entry):[item,...items]);navigate(`/admin/${module}`,{state:{message:`${config.singular} ${editing?'updated':'created'} successfully.`}})}}/>
  const remove=()=>{if(!deleteItem)return;persist(items.filter((item)=>item.id!==deleteItem.id));setDeleteItem(null);setMessage(`${config.singular} deleted successfully.`)}
  const toggle=(item:CmsItem)=>{const status=item.status===config.statuses[0]?config.statuses[1]:config.statuses[0];persist(items.map((entry)=>entry.id===item.id?{...entry,status}:entry));setMessage(`${config.singular} status updated.`)}
...
function ContentEditor({module,existing,onSave}:{module:CmsModule;existing?:CmsItem;onSave:(item:CmsItem)=>void}) {
  const config=configs[module],[form,setForm]=useState<Record<string,string|boolean|string[]>>(()=>existing||{status:config.statuses[0]}),[errors,setErrors]=useState<Record<string,string>>({})
  const update=(name:string,value:string|boolean|string[])=>setForm((current)=>({...current,[name]:value}))
  const submit=(event:FormEvent)=>{event.preventDefault();const next:Record<string,string>={};config.fields.forEach((field)=>{if(field.required&&!form[field.name])next[field.name]=`${field.label} is required.`});setErrors(next);if(Object.keys(next).length)return;onSave({...form,id:existing?.id||crypto.randomUUID(),status:String(form.status||config.statuses[0])} as CmsItem)}
  return <div className="cms-page"><PageHeader eyebrow={existing?'Edit content':'New content'} title={`${existing?'Edit':'Add'} ${config.singular}`} copy="Complete the fields, preview media and choose the publishing status."/><form className="cms-editor" onSubmit={submit}><div className="cms-form-grid">{config.fields.map((field)=><FormField key={field.name} field={field} value={form[field.name]} error={errors[field.name]} onChange={(value)=>update(field.name,value)}/>)}<label className="cms-field"><span>Status</span><select value={String(form.status||config.statuses[0])} onChange={(e)=>update('status',e.target.value)}>{config.statuses.map((status)=><option key={status}>{status}</option>)}</select></label></div><div className="cms-editor-actions"><Link to={`/admin/${module}`} className="cms-secondary-button">Cancel</Link><button className="cms-primary-button" type="submit">{existing?'Save changes':`Create ${config.singular}`}</button></div></form></div>
}
...
export function MediaManager() {
  ...
  const persist=(next:CmsItem[])=>{writeItems('media',next);setItems(next)}
