...
const services: {
...
}[] = [
  {
    ...
    features: ["4K Cinematography", "Professional Editing", "Color Grading", "Sound Design"],
    icon: Clapperboard,
  },
  {
    ...
    features: ["Studio Recording", "Audio Mixing", "Jingle Creation", "Distribution"],
    icon: Mic,
  },
  {
    title: "Video Ads",
    ...
    features: ["Concept Development", "Professional Shooting", "Post Production", "A/B Testing"],
    icon: Video,
  },
  {
  ...
  },
  {
    ...
    features: ["Custom Modules", "Inventory & Finance", "Role-Based Access", "Cloud Deployment"],
    icon: Database,
  },
  {
    ...
    features: ["Market Research", "Campaign Management", "ROI Tracking", "Growth Hacking"],
    icon: TrendingUp,
  },
  {
  ...
  },
  {
    ...
    features: ["Voter Targeting", "Social Media Campaigns", "Message Strategy", "Analytics"],
    icon: Landmark,
  },
  {
    ...
    features: ["Mass Messaging", "Scheduled Campaigns", "2-Way Communication", "Delivery Reports"],
    icon: MessageSquare,
  },
];
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mx-auto max-w-2xl px-2 text-sm text-gray-600 sm:text-base">
            Let's discuss how our services can help you achieve your business goals. Pick
            a service above and get in touch.
          </p>
        </motion.div>
      </div>
    </section>
...
function ServicesGrid({ visibleServices }: { visibleServices: typeof services }) {
  return (
          ...
          >
            ...
            <div className="relative flex items-start justify-between gap-3">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 text-white shadow-sm shadow-orange-500/30 sm:h-14 sm:w-14">
                <Icon className="h-6 w-6 sm:h-7 sm:w-7" strokeWidth={1.85} />
              </span>
            ...
            <div className="relative mt-5 flex flex-wrap gap-1.5 sm:mt-6 sm:gap-2">
              {service.features.map((feature) => (
                <span
                  key={feature}
                  className="rounded-full bg-gray-50 px-2.5 py-1 text-[11px] font-medium text-gray-600 ring-1 ring-inset ring-gray-100 sm:px-3 sm:text-xs"
                >
                  {feature}
                </span>
              ))}
...
function ServicesSidebar({ visibleServices }: { visibleServices: typeof services }) {
  const [activeIndex, setActiveIndex] = useState(0);
  const [touchStartX, setTouchStartX] = useState(0);
  const active = visibleServices[activeIndex];
  const serviceCount = visibleServices.length;

  const goTo = (index: number) => setActiveIndex((index + serviceCount) % serviceCount);
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) (diff > 0 ? nextService : prevService)();
  };
        ...
        {visibleServices.map((service, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon className="h-5 w-5 sm:h-5.5 sm:w-5.5 lg:h-6 lg:w-6" strokeWidth={2} />
              </span>
                ...
                <div className="mt-6 sm:mt-8">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {active.features.map((feature) => (
                      <span
                        key={feature}
                        className="rounded-full bg-white/15 px-3 py-1 text-xs text-white backdrop-blur-sm sm:px-4 sm:py-1.5 sm:text-sm"
                      >
                        {feature}
                      </span>
                    ))}
                    ...
                    {visibleServices.map((_, index) => (
                      <button
                        ...
                        className={`h-2 rounded-full transition-all duration-300 ${
                          activeIndex === index ? "w-6 bg-white" : "w-2 bg-white/40 hover:bg-white/60"
                        }`}
                      />
                    ))}
                  </div>
